...
- Units are `content/hangul.json`, `content/phrases.json` and `content/vocab.json`. A unit contains `lessons`, each with an `id`, a `title`, an optional `icon`, and `items` (at least 4 per lesson).
